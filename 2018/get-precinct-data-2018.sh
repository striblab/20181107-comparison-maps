...
cand_order;cand_name;suffix;incumbent;party;precincts_reporting;\
precincts_voting;votes;votes_pct;votes_office" | \
  cat - <(wget -O - -o /dev/null $GOV14) > mn-gov-18.tmp.csv &&

echo "2018 senate ..." &&
echo "state;county_id;precinct_id;office_id;office_name;district;\
cand_order;cand_name;suffix;incumbent;party;precincts_reporting;\
precincts_voting;votes;votes_pct;votes_office" | \
  cat - <(wget -O - -o /dev/null $SEN14) > mn-sen-18.tmp.csv &&

echo "2018 senate special ..." &&
echo "state;county_id;precinct_id;office_id;office_name;district;\
cand_order;cand_name;suffix;incumbent;party;precincts_reporting;\
precincts_voting;votes;votes_pct;votes_office" | \
  cat - <(wget -O - -o /dev/null $SEN14) > mn-senspec-18.tmp.csv &&
...
ndjson-join 'd.id' - <(cat joined-sen-18.tmp.ndjson) | \
  ndjson-map '{"id": d[0].id, "winner16pres": d[0].winner16pres, "winner18gov": d[0].winner14gov, "winner18sen": d[1].winner, "votes16pres": d[0].votes16pres, "votes18gov": d[0].votes14gov, "votes18sen": d[1].total_votes}' | \
ndjson-join 'd.id' - <(cat joined-senspec-18.tmp.ndjson) | \
  ndjson-map '{"id": d[0].id, "winner16pres": d[0].winner16pres, "winner18gov": d[0].winner14gov, "winner18sen": d[0].winner, "winner18senspec": d[1].winner, "votes16pres": d[0].votes16pres, "votes18gov": d[0].votes14gov, "votes18sen": d[0].total_votes, "votes18senspec": d[1].total_votes}' > joined-all.tmp.ndjson
...
ndjson-split 'd.objects.precincts.geometries' < mn-precincts-longlat.tmp.json | \
  ...
  ndjson-join 'd.properties.id' 'd.id' <(cat mn-precincts-longlat.tmp.ndjson) <(cat joined-all.tmp.ndjson) | \
   ndjson-map '{"type": d[0].type, "arcs": d[0].arcs, "properties": {"id": d[0].properties.id, "county": d[0].properties.county, "precinct": d[0].properties.precinct, "area_sqmi": d[0].properties.area_sqmi, "winner16pres": d[1].winner16pres, "winner18gov": d[1].winner18gov, "winner18sen": d[1].winner18sen, "winner18senspec": d[1].winner18senspec, "votes18_sqmi": d[1].votes18gov / d[0].properties.area_sqmi, "r16_dgov18": d[1].winner16pres == "R" && d[1].winner18gov == "DFL" ? "y" : "n", "r16_dsen18": d[1].winner16pres == "R" && d[1].winner18sen == "DFL" ? "y" : "n", "r16_dsenspec18": d[1].winner16pres == "R" && d[1].winner18senspec == "DFL" ? "y" : "n", "dsen18_rsenspec18": d[1].winner18sen == "DFL" && d[1].winner14senspec == "R" ? "y" : "n"}}' | \
...
echo "Cleaning up" &&
rm *.tmp.* &&
rm mn-precincts-final-2018.json